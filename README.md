...
| `irq` | Enable CPU-level IRQ delivery (activate hardware reflexes) |
...
TTBR0_EL1 — single identity map covering 0x00000000–0xFFFFFFFF:
  ...
  0x40000000  Kernel image (code, data, stacks,
              page tables) up to __kernel_end   Normal WB, EL1 RW, EL0 none
  0x40200000  Sandbox scratch (2 MB block)      Normal WB, EL1+EL0 RW, no-exec
```
...
After `mmu_enable()` returns, both caches are active and the full WB inner-shareable attributes are in effect.
...
Then from the shell:
```
irq       ← enables CPU-level IRQ delivery
```
